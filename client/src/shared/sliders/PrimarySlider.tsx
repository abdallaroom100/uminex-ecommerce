  ...
  return (
    <div className=" h-full  ">
      <Swiper
        ...
        breakpoints={{
          1550: { slidesPerView: xllScreen || 5 },
          1300: { slidesPerView: xlScreen || 4 },
          991: { slidesPerView: lgScreen || 3 },
          450: { slidesPerView: mdScreen || 2 },
        }}
        pagination={{
          clickable: true,
        }}
        ...
        {data.map((item, idx) => {
          return (
            <SwiperSlide
              ...
              className=" max-w-fit group bg-white p-5   pb-0 rounded-sm"
            >
              <div className="slide__container relative ">
                {item.discound > 0 && <div className="discount__sign absolute flex justify-center z-10 top-0 left-0 text-[14px] px-4 rounded-sm py-1 bg-red-500 text-white w-11">
                  -{item.discound}%
                </div>}
                ...
                >
                  ...
                  <div className="price__bundle flex gap-1 items-end">
                    <p className={`${item.discound ? "text-red-500":"text-blue-600"}  mr-1`}>${item.price}.00 
                  </p>
                  {item.discound  > 0 && <span className="text-[15px] text-[#8d979e]">
                      <del>${Math.floor(item.price - item.price*item.discound/100)}.00</del>
                    </span>}
                  ...
                  {item.quantityLimit ? (
                    ...
                    <div className="stock__status flex items-center justify-start gap-1 ">
                      <IoMdCheckmark className={`${item.quantity > 0 ? "text-green-500":"text-red-500"}`} size={15}/>
                       <p className={`text-[12px]  ${item.quantity > 0 ? "text-green-500":"text-red-500"}`}> {item.quantity > 0 ? "in stock":"out of stock"}</p>
                    </div>
                  )}
                  ...
                  >
                    <button className={`px-13 py-2 primary__transition text-white text-[13px] outline-none cursor-pointer font-[600] ${item.quantity > 0 ? "bg-blue-600":"bg-red-600"}  rounded-full`}>
